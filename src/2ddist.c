...
void main (int argc, char * argv[])
{
...
#ifndef DOUBLE_PRECISION
    sscanf(scr_line_,"%f %f %f %f",&data[nData].x1,&data[nData].x2,&data[nData].x3,&data[nData].x4);
#else
    sscanf(scr_line_,"%lf %lf %lf %lf",&data[nData].x1,&data[nData].x2,&data[nData].x3,&data[nData].x4);
#endif
    if (autoscale) {
  ...
  fprintf(stderr,"#nData %i nNormData %i: x1 [%.3lf,%.3lf,%.5lf]; x2 [%.3lf,%.3lf,%.5lf]\n",

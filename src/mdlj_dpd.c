/* 
   ...
   in a periodic boundary using the Dissipative
...
void usage ( void ) {
  fprintf(stdout,"mdlj_dpd usage:\n");
  fprintf(stdout,"mdlj_dpd [options]\n\n");
...
double total_e ( double * rx, double * ry, double * rz, 
		 ...
		 double gamma, double sigma, double sqrt_dt, gsl_rng * r,
		 double * vir ) {
   int i,j;
...
int main ( int argc, char * argv[] ) {
  ...
  int N=216,c,a;
  double L=0.0;
  double rho=0.5, Tb = 1.0, gamma=1.0, sigma, sqrt_dt, rc2 = 1.e20;
  ...
  /* compute the square and square-root of time step */
  dt2=dt*dt;
  sqrt_dt = sqrt(dt);

  /* Compute sigma from gamma for DPD thermostat */
  sigma = sqrt(2*gamma*Tb);

  /* Output some initial information */
  fprintf(stdout,"# DPD-Thermostat MD Simulation"
  ...
  PE = total_e(rx,ry,rz,vx,vy,vz,fx,fy,fz,N,L,rc2,ecor,ecut,
	       gamma,sigma,sqrt_dt,r,&vir);
  TE0=PE+KE;

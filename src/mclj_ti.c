/* 
   ...
   in a periodic boundary using the modified pair potential
   
   U(r,\lambda) = 4(\lambda^5 r^{-12} - \lambda^3 r^{-6})

   for all pairs involving atom N-1;

   to calculate the ensemble average of 

   \frac{\partial U}{\partial\lambda}

   for use in thermodynamic integration.
...
/* An N^2 algorithm for computing the total energy.  The virial
   is also computed and returned in *vir. <dU/dLambda> is
   also accumulated in *dudl  */
double total_e ( double * rx, double * ry, double * rz, int N, double L,
   ...
   *vir=*dudl=0.0;
   for (i=0;i<(N-1);i++) {
	...
	if (r2<rc2) {
	  r6i   = 1.0/(r2*r2*r2);
	  // particle N-1 is the ideal-gas particle
	  if (j==(N-1)) {
...
int main ( int argc, char * argv[] ) {
  ...
  int N=108,c,a;
  double L=0.0;
  double rho=0.5, T=1.0, rc2 = 2.5*2.5, vir, vir_old, vir_sum, pcor, V;
  ...
  if (!short_out) {
    ...
    fprintf(stdout,"# using the lambda-LJ potential\n");
  ...
  init(rx,ry,rz,N,L,r);
  E_old = total_e(rx,ry,rz,N,L,rc2,lambda,tailcorr,ecor,
		  shift,ecut,ecut_l,decutdl,&vir_old,&dudl_old);
  ...
  if (short_out)
    ...
    fprintf(stdout,"NVT Metropolis Monte Carlo Simulation"
	    ...
	    "<dU/dLambda>:                     %.5lf\n"
	    "Program ends.\n",
	    N,nCycles,sqrt(rc2),dr,rho,1.0/T,lambda,
